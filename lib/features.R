...
extract_feature <- function(tesseract_list, truth_set, grams, freq_bigrams){
  ...
  for (i in 1:length(tesseract_list)){
    ...
    # return(matrix(unlist(lapply(tesseract_list[[i]], cur_token_features, unlist(tesseract_list), truth_set, freq_bigrams)), byrow = TRUE, ncol = 23))
...
#   feature_mat[, -4] <- matrix(unlist(lapply(tesseract_vec, cur_token_features, tesseract_vec, truth_set)), byrow = TRUE, ncol = 3)
#   
#   n <- 5
#   for (i in 1:len){
#     # cur_token <- tesseract_vec[i]
#     # 
#     # #f1 Presence of non alpha-numeric text within a word
#     # f1 <- 0
#     # l <- nchar(cur_token)
#     # if ( l > 3){
#     #   bulk <- substr(cur_token, start = 1, stop = l-1)
#     #   s <- str_count(bulk, pattern = "[^[:alnum:]\\\"\\']")
#     #   f1 <- s/l
#     # }
#     # 
#     # #f2 1-gram frequency of a word
#     # f2 <- sum(tesseract_vec == cur_token)/2
#     # 
#     # 
#     # 
#     # #f3
...
#     # }
#     
#     #f4 sliding window, 5-gram freq
#     f4 <- 0
# 
#     
#     # slide_freq <- rep(list(0), n)
#     slide_window <- rep(list(NULL), n)
#     
#     if(i >= 5){slide_window[[1]] <- paste(low_tesseract_vec[(i-4):i], collapse = " ")}
#     if(i >= 4 & i <= len - 1){slide_window[[2]] <- paste(low_tesseract_vec[(i-3):(i+1)], collapse = " ")}
#     if(i >= 3 & i <= len - 2){slide_window[[3]] <- paste(low_tesseract_vec[(i-2):(i+2)], collapse = " ")}
#     if(i >= 2 & i <= len - 3){slide_window[[4]] <- paste(low_tesseract_vec[(i-1):(i+3)], collapse = " ")}
#     if(i <= len - 4){slide_window[[5]] <- paste(low_tesseract_vec[i:(i+4)], collapse = " ")}
#     
#     slide_freq <- unlist(lapply(slide_window, count_slide, gram_vec))
#     
#     f4 <- max(slide_freq, na.rm = TRUE)
#     
#     feature_mat[i,4] <- f4
#   }
#   return(feature_mat)
