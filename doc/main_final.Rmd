...
```{r}
accuracy_with_cand <- sum(correction == test_truth)/length(correction)
print(accuracy_with_cand)

overall_accuracy <- sum(correction == test_truth)/nrow(test_data)
print(overall_accuracy)
```
### adaboost
```{r}
accuracy_with_cand <- sum(correction1 == test_truth1)/length(correction1)
print(accuracy_with_cand)

overall_accuracy <- sum(correction1 == test_truth1)/nrow(test_data)
print(overall_accuracy)
```
...
# old interaction
old_intersect_vec <- vecsets::vintersect(train_data$Truth,train_data$tess)
# new interaction
new_intersect_vec <- vecsets::vintersect(dat$true_token[dat$label == 0],  dat$corrected_token[dat$label == 0])
