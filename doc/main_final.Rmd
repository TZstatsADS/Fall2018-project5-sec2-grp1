...
packages.used <- c("devtools","vwr","kernlab", "rlist", "utils", "ggplot2","qualV","rapportools","xgboost","JOUSBoost")
...
# test_data <- read.csv('../output/clean_test_error.csv')
...
### ground trurh vector length and tesseract vector length
tesslength <- 0
for (i in 1:20){
  c <- length(test_tokens_list[[i]])
  tesslength <- tesslength+c
}
...
OCR_performance_table["word_wise_recall","Tesseract"] <- length(old_intersect_vec)/tesslength
OCR_performance_table["word_wise_precision","Tesseract"] <- length(old_intersect_vec)/tesslength
OCR_performance_table["word_wise_recall","Tesseract_with_postprocessing"] <- length(new_intersect_vec2)/tesslength
OCR_performance_table["word_wise_precision","Tesseract_with_postprocessing"] <- length(new_intersect_vec2)/tesslength

# character-level evaluation
# old interaction
ground_truth_vec_char <- str_split(paste(unlist(test_tokens_list), collapse = ""), "")[[1]]
